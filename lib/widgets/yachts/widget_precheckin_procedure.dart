...
import 'package:boatrack_mobile_app/models/pre_post_prep_segment.dart';
...
class YachtPreCheckInProcedure extends StatefulWidget {
  final Yacht yacht;

  const YachtPreCheckInProcedure({Key? key, required this.yacht})
      : super(key: key);

  @override
  State<YachtPreCheckInProcedure> createState() =>
      _YachtPreCheckInProcedureState();
}

class _YachtPreCheckInProcedureState extends State<YachtPreCheckInProcedure> {
  double width = 0;
  double height = 0;
  List<PrePostSegment> segments = getPreSegments();
  ...
  void sendData() async {
    ...
    if (success) {
      GlobalSnackBar.show(
          context, "Completed pre checkin prep " + widget.yacht.name.toString());
    }
